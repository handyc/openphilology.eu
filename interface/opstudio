...
def ngrams(request):
    ...
    latest_witness_list = Witness.objects.order_by('name')
    ...
    nsize_filter1=usersettings.nsize1
    nsize_filter2=usersettings.nsize2

    latest_ngrams_list1 = NgramInWitness.objects.filter(witness_id=witness1_id).order_by('-weight')[:nsize_filter1]
    latest_ngrams_list2 = NgramInWitness.objects.filter(witness_id=witness2_id).order_by('-weight')[:nsize_filter2]
    ...
    if request.POST:
        ...
        if(reallypost == "1"):
            alignment = Alignment.objects.create(witness_src_id=w_src_id, witness_dst_id=w_dst_id, char_start_src=cs_src, char_end_src=ce_src, char_start_dst=cs_dst, char_end_dst=ce_dst, score=sc, type_id=t_id, tentative_name=segment_name, owner_id=current_user.id)
...
def alignments(request):
    ...
    if request.POST:
        ...
        latest_ngrams_list1 = NgramInWitness.objects.filter(witness_id=witness1_id).order_by('-frequency')[:nsize_filter1]
        latest_ngrams_list2 = NgramInWitness.objects.filter(witness_id=witness2_id).order_by('-frequency')[:nsize_filter2]
        #latest_ngrams_list1 = NgramInWitness.objects.filter(witness_id=witness1_id).order_by('-nsize')[:nsize_filter]
        #latest_ngrams_list2 = NgramInWitness.objects.filter(witness_id=witness2_id).order_by('-nsize')[:nsize_filter]
        #latest_ngrams_list1 = NgramInWitness.objects.filter(witness_id=witness1_id).order_by('frequency')[:nsize_filter]
        #latest_ngrams_list2 = NgramInWitness.objects.filter(witness_id=witness2_id).order_by('frequency')[:nsize_filter]
        ...
        if form.is_valid():
            ...
            for row in srcreader:
                ...
                if matchtib and matchzh:
                    ...
                    if Alignment.objects.filter(witness_src_id=witness_src, witness_dst_id=witness_dst, char_start_src=w1match, char_start_dst=w2match, type_id=2).exists():
...
def add_alignment(request):
    ...
    if request.method == 'POST':
        ...
        alignment = Alignment.objects.create(witness_src_id=witness_src, witness_dst_id=witness_dst, char_start_src=this1match, char_end_src=this1end, char_start_dst=this2match, char_end_dst=this2end, score=score, type_id=2, tentative_name="csv_addition", owner_id=current_user.id)        
        #alignment = Alignment.objects.create(witness_src_id=w_src_id, witness_dst_id=w_dst_id, char_start_src=cs_src, char_end_src=ce_src, char_start_dst=cs_dst, char_end_dst=ce_dst, score=sc, type_id=t_id, tentative_name=segment_name, owner_id=current_user.id)
        #alignment_log = AlignmentLog.objects.create(alignment_id=alignment.id, witness_src_id=w_src_id, witness_dst_id=w_dst_id, char_start_src=cs_src, char_end_src=ce_src, char_start_dst=cs_dst, char_end_dst=ce_dst, score=sc, type_id=t_id, user_id=current_user.id)
        #alignment_log = AlignmentLog.objects.create(alignment_id=alignment.id, witness_src_id=witness_src, witness_dst_id=witness_dst, char_start_src=cs_src, char_end_src=ce_src, char_start_dst=cs_dst, char_end_dst=ce_dst, score=sc, type_id=t_id, user_id=current_user.id)
...
def update_alignment(request):
    ...
    try:
        provisionaldigit = int(post['provisionaldigit'])
    except ValueError:
        provisionaldigit = None

    try:
        provisionalletter = str(post['provisionalletter'])
    except ValueError:
        provisionalletter = None
    if provisionalletter == "":
        provisionalletter = None
    ...
    if provisionaldigit != None:
        if provisionalletter != None:
            one_entry.tentative_name = str(provisionaldigit) + "." + str(provisionalletter)
        else:
            one_entry.tentative_name = str(provisionaldigit)
...
def settings(request):
    ...
    if request.POST:
        ...
        nsize1 = int(post['nrange1'])
        nsize2 = int(post['nrange2'])
        ...
        if(reallypost == "2"):
            ...
            usersettings.nsize1 = nsize1
            usersettings.nsize2 = nsize2
